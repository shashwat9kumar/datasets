...
class RankingExample(NamedTuple):
  """Represents a parsed ranking example for a given query identifier.

  The ranking examples are represented as named tuples because the TFDS datasets
  that use this parser expect tuples of (identifier, features) as the return
  type.
  ...
  """
  qid: int
  features: Mapping[str, Union[np.ndarray, List[float], List[int]]]
...
class LibSVMRankingParser(Iterable[RankingExample]):
  ...
  def _add_to_current_example(self, qid: int, feature_map: Mapping[str, float]):
    ...
    if self._current_example is None:
      self._reset_current_example(qid)
  ...
  def _reset_current_example(self, qid: int):
    """Resets the current example to an empty one with given qid.

    Args:
      qid: The query identifier to use for the new example.
    """
  ...
  def _store_current_example(self):
    ...
    This method converts the current example to numpy format and stores it in
    `self._available_examples` so that it can be yielded by `self.__iter__`.
    """
    ...
    self._available_examples.append(RankingExample(qid, np_features_dict))
