...
RankingExample = collections.namedtuple("RankingExample", ("qid", "features"))
...
class LibSVMRankingParser(Iterable[RankingExample]):
  """A parser for LibSVM-formatted ranking files.

  This parser can parse an iterable of lines into an iterable of
  `RankingExample`s that contain the per-query labels and features.

  Example usage:
  >>> lines = ["1 qid:1 1:5.0 2:3.0", "2 qid:1 1:2.0 2:1.0"]
  >>> names = {1: "bm25", 2: "tfidf"}
  >>> parser = LibSVMRankingParser(lines, names)
  ...
  def _parse_line(self, line_number: int, line: str):
    ...
    new item to be made available in `self._items`.
    ...
    try:
      label, qid, *features = re.split(r"\s+", line.strip())
    except ValueError as value_error:
      raise ParserError(
          line_number, line,
          "could not extract label, qid and features") from value_error

    # Convert relevance label to int.
    try:
      label = int(label)
    except ValueError as value_error:
      raise ParserError(
          line_number, line,
          f"label '{label}' could not be converted to an int") from value_error

    # Convert qid to int.
    if qid[:4] != "qid:":
      raise ParserError(line_number, line,
                        f"line must contain a qid after the relevance label")
    try:
      qid = int(qid[4:])
    except ValueError as value_error:
      raise ParserError(
          line_number, line,
          f"qid '{qid[4:]}' could not be converted to an int") from value_error
    ...
    self._add_to_example(qid, feature_dict)

  def _add_to_example(self, qid: int, feature_dict: Mapping[str, float]):
    """Adds given qid and feature_dict to the current example.

    If the qid matches the current example qid, this will add the features in
    given feature_dict to the current example.
    If the qid does not match the current example qid, this will store the
    current example and then create a new one with given qid and feature_dict.

    Args:
      qid: The qid
      feature_dict: The dictionary of features and values.
    """
    if self._current is None:
      self._reset(qid)

    if self._current.qid != qid:
      self._store_current()
      self._reset(qid)

    for key, value in feature_dict.items():
      self._current.features[key].append(value)

  def _reset(self, qid: int):
    """Resets the current example to an empty one with given qid.

    Args:
      qid: The qid to use for the new example.
    """
    self._current = RankingExample(qid, collections.defaultdict(list))

  def _store_current(self):
    """Converts the current ranking example to np format and stores it."""
    qid = self._current.qid
    np_features_dict = {
        key: np.array(value) for key, value in self._current.features.items()
    }
    self._items.append(RankingExample(qid, np_features_dict))
  ...
  def __iter__(self):
    """Iterates over `RankingExample`s.

    Yields:
      `RankingExample` that represent a single query.

    Raises:
      ParserError: If parsing failed.
    """
    # Parse each line and yield the resulting items as they become available.
