...
You can specify whether to use the "10k" or "30k" version of the dataset as
follows:

```python
ds = tfds.load("mslr_web/30k")
```

If only `mslr_web` is specified, the `mslr_web/10k` option is selected by
default:
```python
# This is the same as `tfds.load("mslr_web/10k", split="fold1_train")`
...
class MslrWebConfig(tfds.core.BuilderConfig):
  size: str = "10k"  # should be one of "10k" or "30k".
...
class MslrWeb(tfds.core.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      MslrWebConfig(name="10k", size="10k"),
      MslrWebConfig(name="30k", size="30k")
  ]
  ...
  def _info(self) -> tfds.core.DatasetInfo:
    ...
    features[_LABEL_NAME] = tfds.features.Tensor(shape=(None,), dtype=tf.int64)
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    return {
        f"fold{fold}_{split}":
        self._generate_examples(path / f"Fold{fold}/{split}.txt")
        for fold in [1, 2, 3, 4, 5] for split in ["train", "vali", "test"]
    }
