...
class LibSVMRankingParserTest(testing.TestCase):
  ...
  def test_parses_a_libsvm_ranking_dataset(self):
    ...
    results = iter(
        LibSVMRankingParser(
            _TEST_DATASET.split("\n"),
            feature_names={n: f"feature_{n}" for n in range(1, 6)}))

    qid, features = next(results)
    self.assertEqual(qid, 1)
    self.assertAllEqual(features["label"], [3, 1, 0, 0])
    self.assertAllEqual(features["feature_1"], [1., 0., 0., 0.])
    self.assertAllEqual(features["feature_2"], [1., 0., 1., 0.])
    self.assertAllEqual(features["feature_3"], [0., 1., 0., 1.])
    self.assertAllEqual(features["feature_4"], [0.2, 0.1, 0.4, 0.3])
    self.assertAllEqual(features["feature_5"], [0., 1., 0., 0])

    qid, features = next(results)
    self.assertEqual(qid, 2)
    self.assertAllEqual(features["label"], [0, 1, 0, 0, 1, 2])
    self.assertAllEqual(features["feature_1"], [0., 1., 0., 0., 0., 1.])
    self.assertAllEqual(features["feature_2"], [0., 0., 0., 0., 0., 1.])
    self.assertAllEqual(features["feature_3"], [1., 1., 1., 1., 1., 0.])
    self.assertAllEqual(features["feature_4"], [0.2, 0.4, 0.1, 0.2, 0.1, 0.3])
    self.assertAllEqual(features["feature_5"], [0., 0., 0., 0., 1., 0.])

    qid, features = next(results)
    self.assertEqual(qid, 3)
    self.assertAllEqual(features["label"], [3, 0])
    self.assertAllEqual(features["feature_1"], [1., 0.])
    self.assertAllEqual(features["feature_2"], [0., 1.])
    self.assertAllEqual(features["feature_3"], [0., 1.])
    self.assertAllEqual(features["feature_4"], [0.4, 0.5])
    self.assertAllEqual(features["feature_5"], [1., 0.])
  ...
  def test_raises_error_if_line_only_contains_label(self):
    ...
    with self.assertRaisesRegex(ParserError,
      ...
      for item in LibSVMRankingParser(contents, {}):
        pass

  def test_raises_error_if_label_is_not_a_number(self):
    contents = ["abc qid:1 2:0.0 3:0.0"]
    with self.assertRaisesRegex(ParserError,
                                "label 'abc' could not be converted to an int"):
      for item in LibSVMRankingParser(contents, {}):
        pass
  ...
  def test_raises_error_if_qid_is_malformatted(self):
    contents = ["1 qid:ABCD 2:0.0 3:0.0"]
    with self.assertRaisesRegex(ParserError,
                                "qid 'ABCD' could not be converted to an int"):
      for item in LibSVMRankingParser(contents, {}):
        pass
  ...
  def test_raises_error_with_offending_line_number(self):
    contents = ["1 qid:1 2:0.0", "1 qid:ERROR 2:0.0", "1 qid:1 2:0.0"]
  ...
  def test_raises_error_with_offending_line(self):
    contents = ["1 qid:1 2:1000.0", "2 qid:1 9:9.9", "1 qid:ERROR 0:0.0"]
    with self.assertRaisesWithPredicateMatch(
        ParserError, lambda error: error.line == "1 qid:ERROR 0:0.0"):
      for item in LibSVMRankingParser(contents, {}):
        pass
